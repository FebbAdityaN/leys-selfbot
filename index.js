	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				var namea = client.contacts[num] != undefined ? client.contacts[num].vname || client.contacts[num].notify : 'unknown'
				try {
					ppimg = await client.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/welcome?name=${namea}&group=${mdata.subject}&profile=${ppimg}&apikey=${setting.apiKey}`)
				...
				buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/leave?name=${namea}&group=${mdata.subject}&profile=${ppimg}&apikey=${setting.apiKey}`)
...
_Powered by nodejs 14.x.x_`
					...
					break
				case 'lirik':
					anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/lirik?q=${q}&apikey=${apiKey}`)
					client.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					break
				case 'soundcloud':
					anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/scdl?q=${q}&apikey=${apiKey}`)
				...
				case 'joke':
					...
					break
				case 'resep':
					anu = await fetchJson(`http://leyscoders-api.herokuapp.com/api/resep?q=${q}&apikey=${apiKey}`)
					teks = '=================\n'
					no = 0
					for (let v of anu.result) {
						no += 1
						teks += `HASIL KE - ${no.toString()}\n• Title: ${v.title}\n• Waktu: ${v.time}\n• Porsi: ${v.servings}\n• Kesulitan: ${v.difficulty}\n• Url: ${v.url}\n• Img: ${v.img}\n=================\n`
					}
					reply(teks)
					break
				case 'kiryuu':
